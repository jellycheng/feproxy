...
企业微信授权-新应用： http://domain.com/feproxy/index.html?proxy_type=workweb&appid=企业ID&agentid=应用ID&redirect_uri=跳转地址&state=&scope=
钉钉授权： http://domain.com/feproxy/index.html?proxy_type=dingtalk&appid=钉钉的AppKey&scope=授权范围&redirect_uri=跳转地址&state=
	参数说明：
	  proxy_type： 代理类型，wxh5-微信h5、wxpc-微信pc、
	  			workweb-企业微信pc（新应用）、workpc-企业微信pc（老应用）、
				dingtalk-钉钉
